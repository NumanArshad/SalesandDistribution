...
const useStyles = makeStyles(theme => ({
 ...
 function AddInventory(props) {
	...
	const [productObj, setProductAttribute] = useState({name:'',price:0,duration:0,storage:0,
	users:0});
...
<Grid item lg={4} md={4} xs={12}>
			  <TextField 
			   fullWidth label="Carton-Price" className={classes.textField} 
	...
	<Grid item lg={4} md={4} xs={12}>
				  <TextField  fullWidth label="Carton-Price" className={classes.textField} 
				//   onChange={(event)=>setPackageAttribute({...packageObj,storage:event.target.value})}
                  />
			  </Grid>
